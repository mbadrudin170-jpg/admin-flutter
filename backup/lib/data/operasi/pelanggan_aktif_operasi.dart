...
class PelangganAktifOperasi {
  ...
  // FUNGSI BARU UNTUK SOFT DELETE
  Future<void> updateSyncStatusToDeleted(String id) async {
    final db = await dbHelper.database;
    ...
    await db.update(
      'pelanggan_aktif',
      {
        ...
        'diperbarui': DateTime.now().toIso8601String(), // Catat waktu perubahan
      },
      where: 'id = ?',
      whereArgs: [id],
    );
    ...
    // Batalin notifikasi yang terkait
    await _notifikasiServis.batalNotifikasi((id.hashCode.abs() + 1));
    await _notifikasiServis.batalNotifikasi((id.hashCode.abs() + 2));
  }

  // Diubah untuk menggunakan fungsi soft delete yang baru
  Future<void> hapusPelangganAktif(String id) async {
    await updateSyncStatusToDeleted(id);
  }
  ...
  Future<void> _jadwalkanNotifikasi(PelangganAktif pelangganAktif) async {
    ...
    final tigaHariSebelum = pelangganAktif.tanggalBerakhir.subtract(
      const Duration(days: 3),
    );
    if (tigaHariSebelum.isAfter(DateTime.now())) {
      await _notifikasiServis.jadwalNotifikasi(
        id: (pelangganAktif.id.hashCode.abs() + 1),
        title: '⏰ Paket Akan Berakhir',
        body: 'Paket $namaPelanggan akan berakhir dalam 3 hari.',
        jadwal: tigaHariSebelum,
      );
    }

    final waktuKadaluarsa = pelangganAktif.tanggalBerakhir;
    if (waktuKadaluarsa.isAfter(DateTime.now())) {
      await _notifikasiServis.jadwalNotifikasi(
        id: (pelangganAktif.id.hashCode.abs() + 2),
        title: '🔔 Paket Berakhir Sekarang',
        body: 'Paket $namaPelanggan telah berakhir hari ini.',
